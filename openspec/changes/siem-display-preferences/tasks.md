...
## 8. Column Visibility For Supported Tables
...
## 10. Tests To Add During Implementation

- [ ] 10.1 Unit test the shared timestamp helper: default (local/24h), UTC selection, 12-hour selection, invalid/missing value fallback.
- [ ] 10.2 Unit test the centralized severity color function: default preset matches prior colors on both the 3-tier and 4-tier call sites; preset switching; resolved critical/high vocabulary.
- [ ] 10.3 Unit/component test `rowsPerPage` on each of the three tables: `"all"` shows every row; a numeric value truncates only rendered rows, applied after filter/sort.
- [ ] 10.4 Component test Live Logs font size scale and immediate application.
- [ ] 10.5 Component test default Live Logs tab initialization and persistence across a simulated reload, plus unaffected manual switching.
- [ ] 10.6 Component test column visibility per table: default all-visible, independent persistence per table, non-hideable `ID` column.
- [ ] 10.7 Component test Live Log highlighting: no rules by default; matching rule applies visual treatment; non-mutation of underlying event data; newly streamed events are evaluated against active rules.
- [ ] 10.8 Regression test that `display` settings recover safely from malformed/partial localStorage without crashing Alerts, Events, Live Logs, or Incident views.
- [ ] 10.9 Regression test that no `/events/search`, `/alerts`, or other API request changes shape/parameters as a result of any setting in this spec.

## 11. Validation To Run During Implementation

- [ ] 11.1 Run focused frontend tests for the shared timestamp helper and its consumers.
- [ ] 11.2 Run focused frontend tests for the centralized severity color function and its consumers.
- [ ] 11.3 Run focused frontend tests for rows-per-page, column visibility, Live Logs font size/default tab, and highlighting.
- [ ] 11.4 Run existing Alerts, Live Logs, and Incidents test suites to confirm no regression.
- [ ] 11.5 Run `npm test -- --watchAll=false` from `frontend/` if practical for the implementation change.
- [ ] 11.6 Run `git diff --check`.

## 12. Out Of Scope

- [ ] 12.1 Do not add backend settings APIs, query-limit changes, or pagination endpoints.
- [ ] 12.2 Do not add database persistence or migrations.
- [ ] 12.3 Do not change authentication, sessions, or RBAC.
- [ ] 12.4 Do not add theme switching or a light theme.
- [ ] 12.5 Do not add column visibility to tables outside the three named in this spec.
- [ ] 12.6 Do not implement alert sound or browser notification preferences from `siem-alert-notification-preferences`.
- [ ] 12.7 Do not modify `siem-settings-foundation` or `siem-settings-roadmap` beyond reading their established schema/contract.
- [ ] 12.8 Do not touch the VM.
