...
- [x] Confirm an index exists that covers `(status, expires_at)` or a partial index on
...
- [x] If no suitable index exists: add one to `schema.sql`:
  ...
  ```
- [x] Run `pytest tests/ -x -q` — passes (schema-only change if needed, no code touched).
...
- [x] Add the following function after `skip_next_terminal_approval_action`:
...
- [x] Import `sweep_terminal_approval_queue_rows` in the test file.

- [x] **Test: empty queue returns empty list**
  - No queue rows present. `sweep_terminal_approval_queue_rows(conn)` returns `[]`.

- [x] **Test: pending queue row (non-awaiting) is not swept**
...
- [x] **Test: awaiting_approval row with approved approval is not swept**
...
- [x] **Test: awaiting_approval row with expired approval is swept to skipped**
...
- [x] **Test: approved/denied requests are not affected**
...
- [x] Add imports at the top of `admin_routes.py` (with existing imports):
  ```python
...
- [x] Add the new endpoint after the last existing `admin/soar` endpoint:
...
- [x] **Test: viewer returns 403**
  - Authenticated as viewer role.
  - Response 403.

- [x] **Test: analyst returns 403**
  - Authenticated as analyst role.
  - Response 403.

- [x] **Test: super_admin with no expired approvals returns zero counts**
...
- [x] **Test: endpoint expires overdue pending approvals**
...
- [x] **Test: endpoint sweeps awaiting_approval queue rows**
...
- [x] **Test: endpoint expires approvals AND sweeps queue rows in one call**
...
- [x] **Test: retry_count not incremented by endpoint**
...
- [x] Confirm `routes/approval_routes.py` was NOT modified. GET routes remain read-only.
- [x] Confirm no scheduler, cron, thread, or daemon was introduced.
- [x] Confirm no frontend files were modified.
- [x] Confirm `soar_action_worker.py` was not modified.
- [x] Confirm `approval_store.py` was not modified (only imported in admin_routes).
- [x] Confirm no `retry_count` increment appears in any new code path.
- [x] Confirm `sweep_terminal_approval_queue_rows` does not call `conn.commit()`.

- [x] Run full suite: `pytest tests/ -x -q` — clean.
- [x] Check count: confirm total test count is at or above 409 (the pre-phase baseline).
