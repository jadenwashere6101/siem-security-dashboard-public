...
## 11. MAC AI — Stale Simulation Framing Correction (post-10.3 wording drift)

- [x] 11.1 Update `specs/soar-mode-aware-presentation/spec.md` "Manual queue simulation explanation" requirement and `design.md` decision 2, which still described the batch as evaluated with `SimulationExecutor` only; requirement renamed to "Manual queue batch explanation" and now requires per-mode batch reporting.
- [x] 11.2 Add `engines.soar_action_worker.classify_queue_action_mode` (public, read-only; wraps existing `_queue_outcome_classification`) and an additive `action` field on `_worker_result`; no writer/outcome-event semantics changed.
- [x] 11.3 Make `/admin/soar/worker/run-once` batch summary mode-aware: `summary.by_mode` / `summary.success_by_mode` counts by canonical execution_mode; rename top-level `mode` response field to `requested_executor_mode` (describes the requested executor-injection gate, not a claim about outcomes) so a mixed batch is never globally reported as `simulation`.
- [x] 11.4 Update `SoarQueuePanel.js`: rename "Run simulation batch" → "Process queue batch"; remove "SimulationExecutor only" claims from subtitle/help/tooltip; replace "simulated internally" → "processed internally" and "Simulated success" → "Processed successfully"; render `summary.by_mode` breakdown; retain the truthful no-external-effect warning.
- [x] 11.5 Correct stale wording that implied simulation-only workflows in `PlaybookMetricsPanel.js` (notice + subtitle — component is currently unmounted/dead but wording corrected for consistency with the live `SoarMetricsDashboard.js` notice), `IncidentsPanel.js` (SOAR Timeline notice), `IntegrationStatusPanel.js` (firewall adapter description — now distinguishes real tracking-only SIEM records from the absence of real firewall enforcement), `playbookService.js`, and `PlaybooksPanel.js` error-path fallback text.
- [x] 11.6 Update/add backend tests (`tests/test_soar_worker_admin_run_control.py`: renamed field, mode-aware summary, new mixed-mode batch test, fixed pre-existing stale "Monitoring only" assertion) and frontend tests (`SoarQueuePanel.test.js`, `PlaybookMetricsPanel.test.js`, `IncidentsPanel.test.js`).
- [x] 11.7 Run focused backend suites (26/26, then 146/146 broader), full frontend suite (724/724), `npm run build` (pre-existing eslint warnings only), `git diff --check`, `openspec validate --strict`; visually verify all 5 representative states (internal, tracking-only, simulated, real-capable, mixed batch) in the live local UI against a temporary local mock backend (removed after verification).

Status: **Mac stale-simulation-wording correction complete** (task 11). Parent remains open for VM phases 6–7. Do not archive. Handoffs unchanged: `docs/soar_outcome_evidence_vm_handoff.md`, `docs/soar_metrics_source_mapping.md`.
