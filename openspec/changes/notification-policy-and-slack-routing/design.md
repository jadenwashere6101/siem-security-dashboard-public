...
- Slack integration already exists and is guarded; this change should route into that existing path rather than invent a second delivery mechanism.
...
- Route pfSense and honeypot notifications by existing alert source, with runtime-configurable channel labels.
...
- Routing should key off the existing alert source family, with two initial route keys:
  - `pfsense` -> runtime-configurable Slack channel label for pfSense
  - `honeypot` -> runtime-configurable Slack channel label for honeypot
