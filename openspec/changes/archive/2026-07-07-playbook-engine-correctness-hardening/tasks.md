...
- [x] 2.9 ~~Land 2.1–2.2, 2.3–2.5, and 2.6–2.7 as three independently revertible commits/PRs, per the Migration Plan in `design.md`.~~ **WAIVED — not applicable after merge, closed by project decision (2026-07-07).** This is NOT incomplete implementation. Rationale:
  - All three functional objectives (protected-target parity, canonical action vocabulary, `attempt_count` recovery) were verified delivered in code: `engines/playbook_step_executor.py` calls `require_unprotected_target` before `block_ip` dispatch; `notify_teams` is part of the unified `KNOWN_PLAYBOOK_ACTIONS`/`SUPPORTED_ACTIONS` vocabulary shared by the registry and executor; `mark_stale_execution_for_recovery` in `core/playbook_store.py` increments `attempt_count` alongside `recovery_count`, making the `max_attempts` give-up branch reachable.
  - Validation passed: the full associated backend test suite (`tests/test_playbook_registry.py`, `tests/test_playbook_step_executor.py`, `tests/test_playbook_store.py`) was re-run and passes (156 passed).
  - The three fixes already landed together in a single merged commit (`f615ae6`, "fix: harden playbook engine correctness") rather than as three separate revertible commits/PRs as originally specified. Commit granularity cannot be changed retroactively without rewriting already-published git history, which is out of scope and not something this project does.
  - This OpenSpec change is therefore being closed based on delivered, tested capability rather than on the historical shape of the commit(s) that delivered it.
