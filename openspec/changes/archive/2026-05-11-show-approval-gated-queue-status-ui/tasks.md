...
- [x] Locate the `QUEUE_STATUSES` constant at the top of the file.
- [x] Change from:
  ```javascript
  ...
  ```
- [x] Confirm `QUEUE_STATUS_FILTERS` is derived from `QUEUE_STATUSES` via spread — no
  separate change needed there.

### 1b. Add `awaitingApprovalBadgeStyle` constant

- [x] Add alongside the existing badge style constants (after `skippedBadgeStyle`):
  ```javascript
...
- [x] Add the `awaiting_approval` case between `running` and `success`:
  ```javascript
...
- [x] Locate the `selectedQueueItem` branch inside the detail panel render block.
- [x] After the last `<DetailField>` entry (the `Idempotency Key` field) and inside the
...
- [x] Add alongside the existing style constants (near `detailGridStyle` or at the end of the
  style block):
  ```javascript
...
- [x] Add `awaiting_approval: 1` to the `counts` object and update `total` to `12`:
  ```javascript
  const statusFixture = {
    counts: {
      pending: 2,
...
- [x] Add after `queueDetailFixture`:
  ```javascript
...
- [x] **Test: `awaiting_approval` count tile is shown**
...
- [x] **Test: `awaiting_approval` option exists in status filter dropdown**
  - Same setup as above.
  - After `waitFor`: `screen.getByRole("option", { name: "Awaiting Approval" })` is in the
    document.

- [x] **Test: `awaiting_approval` badge renders in queue list**
  - `loadSoarQueueStatus` resolves with `statusFixture`.
...
- [x] **Test: empty state for `awaiting_approval` filter**
  - `loadSoarQueueStatus` resolves with `statusFixture`.
  - `loadRecentSoarQueueItems` resolves with `{ items: [] }`.
...
- [x] **Test: approval-waiting note visible in detail for awaiting_approval item**
  - `loadSoarQueueStatus` resolves with `statusFixture`.
  - `loadRecentSoarQueueItems` resolves with `{ items: [awaitingApprovalRowFixture] }`.
...
- [x] **Test: approval-waiting note absent for non-awaiting item**
  - `loadSoarQueueStatus` resolves with `statusFixture`.
...
- [x] Run `npm test -- --watchAll=false` — all tests green, including existing ones.

---

## Step 3: Final audit

- [x] Confirm only these files were modified:
  - `frontend/src/components/SoarQueuePanel.js`
  - `frontend/src/components/SoarQueuePanel.test.js`
- [x] Confirm `SoarQueuePanel.js` does not import from `approvalService.js` or call any
  approval endpoint.
- [x] Confirm the detail note has no event handlers, no anchor tags, and no navigation calls.
