...
### One builder owns the complete planner prompt budget

The `agentic_planning` profile ceiling applies to the exact prompt submitted to the gateway. The builder measures mandatory policy, the complete current message, output schema, compact action/strategy contract, safety boundary, request facts, and every bounded entity identity before admitting optional conversation facts. The Ollama gateway/provider path forwards the request prompt unchanged, so its measured framing overhead is zero; this remains an explicit measured input rather than an assumed reserve.

Optional facts are admitted against both the packet bound and the complete serialized prompt in this structural order: deduplicated recent tool results, recent entity-bearing turns, recent conclusions, unresolved questions, analyst corrections, compact summary, other recent turns, recommendations, and statements. Recency and provenance are transport facts, not server interpretations of conversational meaning. Duplicate entity identities and byte-equivalent evidence records are retained once. Final prompt size is checked again immediately before generation.

The single repair uses a separate compact prompt containing the complete current message, original proposal, exact validation errors, compact semantic contract/output schema, all bounded entity identities, and only optional facts that still fit. It never embeds the complete initial prompt. If either mandatory prompt cannot fit, planning returns an auditable `agentic_planner_configuration_error` outcome without a gateway call, workflow fallback, or uncaught route exception.
...
### Entity-bearing evidence lookups remain bound to the resolved entity

Entity selection and conversational reference resolution remain planner-owned. Once a quick evidence plan contains one structured `resolved_entities` item, however, the server treats that identity as a cross-field invariant rather than an optional hint. The selected tool category must support an exact structured binding for that entity type, and `evidence_requirements` must contain the corresponding identity key and identical normalized value. Alert and detection identities use `alert_id`; incidents use `incident_id`; source-IP entities use `source_ip`; recon activities use `activity_id`; and response-registry entities use `registry_id` only for categories whose existing tool schema can express that identity.

The validator rejects a missing identity, a different identity, or an entity/category pair without an established exact binding and sends the precise structural error through the existing one-attempt repair path. It does not inspect the analyst's words or infer an identity. Entityless `fresh_evidence_lookup` plans remain valid broad searches when their ordinary bounded filters are valid.

Translation consumes the validated identity requirement directly. Exact alert lookup becomes `search_alerts(alert_id=<resolved id>)`; exact incident lookup uses the existing incident timeline tool; related-event and response-registry lookups use their existing identity arguments. The existing alert search executor already delegates SQL construction through the canonical alert-filter helper, so exposing its existing `alert_id` filter in the read-tool schema preserves RBAC, bounds, and result-envelope behavior without adding a new tool or rewriting planner meaning after validation.
...
| Plan/prompt over budget | One builder reserves complete mandatory prompt content, admits optional facts against the exact profile ceiling, and rechecks the submitted prompt | Planner prompt builder, planning service, and provider profile guard | Stay within output contract | First, three, and twenty turns; many entities; large evidence; initial and repair stress |
| Repair prompt independently over budget | Repair has its own measured mandatory packet and optional-fact admission; it never nests the initial prompt | Planner repair builder | Correct the original proposal once | Large original proposal, many validation errors, long thread facts |
| Planner budget error becomes HTTP 500 | Configuration/budget failures compile to a truthful non-executing planner outcome with operator metadata | Planner and conversation orchestration boundaries | None after failure | Sync, async, mandatory-only overflow, explicit shortcut |
| Gateway framing causes hidden overflow | The builder reserves measured gateway framing and the exact prompt is checked again immediately before generation | Planner builder, gateway contract test, provider guard | None | Initial and repair requests at the active ceiling |
...
| Resolved entity and evidence lookup diverge | Every entity-bearing quick lookup names the identical structured entity in `evidence_requirements`, uses a compatible category, and preserves that identity in tool arguments | Cross-field plan validator, one-repair path, tool-request translator, SOC tool validator/executor | Resolve the intended entity and propose evidence for that same entity | Missing alert ID, wrong alert ID, exact incident/source/recon/registry binding, entityless newest search |
