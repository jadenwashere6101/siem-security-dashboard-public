...
- [x] Read existing frontend service patterns.
- [x] Add `frontend/src/services/approvalService.js`.
- [x] Implement `listApprovals(filters)`.
...
- [x] Implement `getApproval(id)`.
  - Calls only `GET /approvals/<id>`.
- [x] Implement `submitApprovalDecision(id, { decision, reason })`.
  - Calls only `POST /approvals/<id>/decision`.
  - Sends `decision` as `approved` or `denied`.
  - Handles optional `reason` safely.
- [x] Add service tests for list, detail, approve, deny, optional reason, and API error handling.

## Step 10: Phase 2.5C approval UI component and navigation

- [x] Read existing frontend role/nav/component patterns.
- [x] Add `frontend/src/components/ApprovalsPanel.js`.
- [x] Add approval nav tab for `analyst` and `super_admin` users.
- [x] Do not expose the approval nav tab to viewer users.
- [x] Render approval list.
- [x] Add status filter.
  - Supports `all`, `pending`, `approved`, `denied`, `expired`.
- [x] Add risk/severity filter.
  - Supports `all`, `medium`, `high`, `critical`.
  - Use API-supported filtering when available; otherwise filter safely client-side.
- [x] Add approval detail view.
...
- [x] Add error state.
- [x] Add empty state.
- [x] Show approve/deny controls only for `super_admin` users viewing pending approvals.
- [x] Hide approve/deny controls for analysts.
- [x] Hide approve/deny controls for approved, denied, and expired approvals.
- [x] Add optional reason input for decisions.
- [x] On approve, call `submitApprovalDecision(id, { decision: "approved", reason })`.
- [x] On deny, call `submitApprovalDecision(id, { decision: "denied", reason })`.
- [x] After a successful decision, refresh the selected approval detail and list.
- [x] On failed decision, show an error and do not locally mark the approval as decided.
- [x] Confirm no queue mutation controls were added.
- [x] Confirm no SOAR action execution controls were added.
- [x] Confirm no worker pause/resume or playbook controls were added.

## Step 11: Phase 2.5C frontend tests and verification

- [x] Add component tests for loading, empty, error, analyst read-only view, super admin pending
...
- [x] Run targeted service/component tests, for example:
...
- [x] Run `npm run build`.
- [x] Run backend approval route/store tests if backend imports or route contracts were touched.
- [x] Confirm no backend files changed unless absolutely required.
- [x] Confirm no schema files changed.
- [x] Confirm no worker pause/resume was added.
- [x] Confirm no queue execution behavior changed.
- [x] Confirm no ingest, detection, or correlation files changed.
