...
def _patched_app_db(conn):
    ...
    with patch("routes.alerts_events_routes.get_db_connection", return_value=wrapper), patch(
        "routes.alerts_events_routes.lookup_ip_reputation", return_value=REPUTATION
    ):
        yield
