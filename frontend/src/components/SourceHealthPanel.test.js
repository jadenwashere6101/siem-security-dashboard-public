...
jest.mock("../services/sourceHealthService", () => ({
  loadSourceHealth: jest.fn(),
  loadSourceHealthMetrics: jest.fn(),
}));
...
const makeMetricsPayload = () => ({
  generated_at: "2026-07-12T15:00:00+00:00",
  cache_ttl_seconds: 300,
  windows: { last_hour_start: "2026-07-12T14:00:00+00:00", today_start: "2026-07-12T00:00:00+00:00", timezone: "UTC" },
  sources: SOURCE_METADATA.map((item) => ({
    source: item.source,
    events_last_hour: item.source === "pfsense" ? 15234 : 12,
    events_today: item.source === "pfsense" ? 431209 : 345,
...
beforeEach(() => {
  jest.clearAllMocks();
  jest.useFakeTimers();
  loadSourceHealthMetrics.mockResolvedValue(makeMetricsPayload());
});
...
test("presents authoritative health state with separately loaded historical counters", async () => {
  loadSourceHealth.mockResolvedValue(makePayload());
  ...
  expect(container).toHaveTextContent(/last hour/i);
  expect(container).toHaveTextContent(/today \(utc\)/i);
  expect(container).toHaveTextContent("5,873,421");
});

test("renders health while historical metrics fail independently", async () => {
  loadSourceHealth.mockResolvedValue(makePayload());
  loadSourceHealthMetrics.mockRejectedValue(new Error("metrics timeout"));
  render(<SourceHealthPanel onOpenLiveLogs={() => {}} />);

  expect(await screen.findByTestId("source-health-grid")).toBeInTheDocument();
  expect(screen.getByText("Healthy")).toBeInTheDocument();
  expect(await screen.findByText(/historical event counts unavailable.*metrics timeout/i)).toBeInTheDocument();
  expect(screen.getAllByText("Unavailable").length).toBeGreaterThanOrEqual(3);
});

test("shows a graceful count loading state without delaying health cards", async () => {
  loadSourceHealth.mockResolvedValue(makePayload());
  loadSourceHealthMetrics.mockReturnValue(new Promise(() => {}));
  render(<SourceHealthPanel onOpenLiveLogs={() => {}} />);
