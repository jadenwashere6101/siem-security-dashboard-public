...
const scheduleListFallback = { items: [], limit: 50, playbook_id: null, enabled: null };

export async function listPlaybookSchedules({ playbookId, enabled, limit } = {}) {
  const params = new URLSearchParams();
  if (playbookId) {
    params.set("playbook_id", playbookId);
  }
  ...
  const query = params.toString();
  const res = await fetch(
    buildSiemPath(`/playbook-schedules${query ? `?${query}` : ""}`),
    { credentials: "include" }
  );
  const data = await parseJsonResponse(res, scheduleListFallback);
  if (!res.ok) {
    throw new Error(
      getApiErrorMessage(data, "Unable to load playbook schedules", ["error", "message"])
    );
  }
  return data;
}

export async function getPlaybookSchedule(scheduleId) {
  const res = await fetch(buildSiemPath(`/playbook-schedules/${scheduleId}`), {
    credentials: "include",
  });
  const data = await parseJsonResponse(res, {});
  if (!res.ok) {
    throw new Error(
      getApiErrorMessage(data, "Unable to load playbook schedule", ["error", "message"])
    );
  }
  return data;
}
