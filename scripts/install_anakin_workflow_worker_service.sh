#!/usr/bin/env bash
# Operator-controlled install/update/rollback for Anakin async workflow worker units.

set -euo pipefail

readonly SERVICE_NAME="anakin-workflow-worker.service"
readonly TIMER_NAME="anakin-workflow-worker.timer"
...
usage() {
  cat <<'EOF'
Usage: scripts/install_anakin_workflow_worker_service.sh [OPTIONS]

Install, update, or roll back the Anakin async workflow worker service/timer.
...
The worker executes queued read-only Anakin Deep Investigate, Decision Support,
and Generate Artifact requests. It does not run SOC briefings, Repo Assistant,
preview/confirm endpoints, SOAR actions, or production mutations.
EOF
}
...
preflight() {
  ...
  [[ -f scripts/anakin_workflow_worker.py ]] || die "missing Anakin workflow worker runner"
  [[ -f scripts/run_anakin_workflow_worker_service.sh ]] || die "missing service wrapper"
...
start_timer() {
  ...
  run_sudo systemctl is-enabled "$TIMER_NAME" --quiet
  run_sudo systemctl is-active "$TIMER_NAME" --quiet
