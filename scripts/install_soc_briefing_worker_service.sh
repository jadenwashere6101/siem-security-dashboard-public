#!/usr/bin/env bash
# Operator-controlled install/update/rollback for scheduled SOC briefing worker units.

set -euo pipefail

readonly SERVICE_NAME="soc-briefing-worker.service"
readonly TIMER_NAME="soc-briefing-worker.timer"
...
usage() {
  cat <<'EOF'
Usage: scripts/install_soc_briefing_worker_service.sh [OPTIONS]

Install, update, or roll back the scheduled SOC briefing worker service/timer.
...
The worker is a runtime foundation only. It does not generate briefing content,
send Slack messages, configure providers, or perform production mutations.
EOF
}
...
preflight() {
  ...
  [[ -f scripts/soc_briefing_worker.py ]] || die "missing SOC briefing worker runner"
  [[ -f scripts/run_soc_briefing_worker_service.sh ]] || die "missing service wrapper"
